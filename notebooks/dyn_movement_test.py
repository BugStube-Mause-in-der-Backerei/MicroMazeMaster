...
def generate_mazes(seed):
    ...
    for index in range(NUM_MAZES):
        mazes.append(Maze(MAZE_SIZE[0], MAZE_SIZE[1],
                     seed=random.randint(1, 1000), missing_walls=False))
    random.seed(None)
    return mazes
...
class MazeEnv:
    ...
    def is_dead_end(self, position):
        if (self.mouse.orientation == Orientation.NORTH):
            return not (self.maze.is_valid_move(position, (position[0], position[1] + 1)) and
                        self.maze.is_valid_move(
                        position, (position[0] + 1, position[1])) and
                        self.maze.is_valid_move(
                        position, (position[0] - 1, position[1])))
        elif (self.mouse.orientation == Orientation.EAST):
            return not (self.maze.is_valid_move(position, (position[0] + 1, position[1])) and
                        self.maze.is_valid_move(
                        position, (position[0], position[1] - 1)) and
                        self.maze.is_valid_move(
    ...
    def find_openings(self, position):
        ...
        if (self.mouse.orientation == Orientation.NORTH):
            return [self.maze.is_valid_move(position, (position[0], position[1] + 1)),  # in front
                    self.maze.is_valid_move(
                        position, (position[0] + 1, position[1])),  # right to
                    self.maze.is_valid_move(
                        position, (position[0] - 1, position[1])),  # left to
                    ] if not between_cells else [
                        self.maze.is_valid_move(
                            # in front
                            (position[0], position[1] - 0.5), (position[0], position[1] + 0.5)),
                self.maze.is_valid_move(
                            # right to
                            (position[0], position[1] - 0.5), (position[0] + 1, position[1] - 0.5)),
                self.maze.is_valid_move(
                            # left to
                            (position[0], position[1] - 0.5), (position[0] - 1, position[1] - 0.5)),
                self.maze.is_valid_move(
                            # two ahead
                            (position[0], position[1] + 0.5), (position[0], position[1] + 1.5)),
                self.maze.is_valid_move(
                            # one ahead and right
                            (position[0], position[1] + 0.5), (position[0] + 1, position[1] + 0.5)),
                self.maze.is_valid_move(
            ...
            return [self.maze.is_valid_move(position, (position[0] + 1, position[1])),  # in front
                    self.maze.is_valid_move(
                        position, (position[0], position[1] - 1)),  # right to
                    self.maze.is_valid_move(
                        position, (position[0], position[1] + 1)),  # left to

                    ] if not between_cells else [
                        self.maze.is_valid_move(
                            # in front
                            (position[0] - 0.5, position[1]), (position[0] + 0.5, position[1])),
                self.maze.is_valid_move(
                            # right to
                            (position[0] - 0.5, position[1]), (position[0] - 0.5, position[1] - 1)),
                self.maze.is_valid_move(
                            # left to
                            (position[0] - 0.5, position[1]), (position[0] - 0.5, position[1] + 1)),
                self.maze.is_valid_move(
                            # two ahead
                            (position[0] + 0.5, position[1]), (position[0] + 1.5, position[1])),
                self.maze.is_valid_move(
                            # one ahead and right
                            (position[0] + 0.5, position[1]), (position[0] + 0.5, position[1] - 1)),
                self.maze.is_valid_move(
    ...
    def step(self, action, agent, state_seq, dt=0.5):
        ...
        if not self.hits_wall(self.mouse.position, new_position):
            ...
            while not self.mouse.move(dt):
                incremental_position = self.mouse.position
                incremental_distance = self.distance_to_goal(
                    incremental_position)
                distance_change = prev_distance - incremental_distance
                reward += REWARD_DISTANCE_CHANGE * distance_change

                self.train_model(
                    state=(incremental_position,
                           self.find_openings(incremental_position)),
                    action=action,
                    reward=reward,
                    next_state=(self.mouse.position,
                                self.find_openings(self.mouse.position)),
                    agent=agent,
                    state_seq=state_seq
                )

                prev_distance = incremental_distance
    ...
    def train_model(self, state, action, reward, next_state, agent, state_seq):
        ...
        state_seq = state_seq[1:] + [state]  # Wrap single state in a list for compatibility
...
def train_agents_on_maze(agents, maze_data, training_repeats=10):
    ...
    for i, agent in enumerate(agents):
        ...
        while forward_steps < MAX_STEPS_PER_EPISODE:  # Limit only forward steps
            ...
            position, reward, done, _, openings = env.step(action, agent, state_seq)
            ...
            # Only increment forward_steps for forward action
            if action == 0:  # Replace with the actual index or constant
                forward_steps += 1
...
def test_agent(agent, test_maze):
    ...
    for step in range(MAX_STEPS_PER_EPISODE):
        action = agent.act(state_seq)
        position, reward, done, action_name, openings = env.step(action, agent, state_seq)
        state_seq = state_seq[1:] + [(position, openings)]
