...
# ============ AGENT CLASS ============ #
class Agent:
    def __init__(self, state_size, action_size, learning_rate=LEARNING_RATE, epsilon_decay=EPSILON_DECAY, gamma=GAMMA):
...
def optimize_hyperparameters(population_size, generations, mazes):
    """Optimize hyperparameters using evolutionary algorithm, retraining the model each time."""
    hyperparameter_space = {
        "learning_rate": (0.001, 0.3),  # Range for learning rate
        "epsilon_decay": (0.5, 1),  # Range for epsilon decay
        "gamma": (0.5, 1),           # Range for discount factor
    }
    ...
    population = [
        {
            ...
            "epsilon_decay": random.uniform(*hyperparameter_space["epsilon_decay"]),
            "gamma": random.uniform(*hyperparameter_space["gamma"]),
        }
    ...
    for generation in range(generations):
        ...
        for hyperparameters in population:
            fitness = evaluate_hyperparameters(hyperparameters, mazes)
        ...
        while len(offspring) < population_size // 2:
            ...
            child = crossover_hyperparameters(parent1, parent2, hyperparameter_space)
            mutate_hyperparameters(child, hyperparameter_space, mutation_rate=0.1)
            offspring.append(child)

        # Update population with elites and offspring
...
def evaluate_hyperparameters(hyperparameters, mazes, num_episodes=10, num_runs=5, penalty_weight=10):
    # Initialize a new agent with the given hyperparameters
    agent = Agent(
        state_size=2,
        action_size=4,
    ...
    # Train the agent on all training mazes
    train_agents_across_mazes([agent], mazes, num_episodes)

    # Test the agent's performance after training
    total_reward = 0
    total_steps = 0

    for maze in mazes:  # Evaluate across mazes
        env = MazeEnv(maze)

        for _ in range(num_runs):  # Test multiple runs for consistency
            state_seq = [env.reset()] * SEQUENCE_LENGTH
            steps = 0
            for step in range(MAX_STEPS_PER_EPISODE):
                action = agent.act(state_seq, env)
    ...
    avg_steps = total_steps / (len(mazes) * num_runs)

    # Combine reward and steps into a fitness score
    fitness = total_reward - (penalty_weight * avg_steps)
    return fitness

def crossover_hyperparameters(parent1, parent2, hyperparameter_space):
    """Perform crossover between two sets of hyperparameters."""
    child = {}
    for key in hyperparameter_space:
        child[key] = (parent1[key] + parent2[key]) / 2  # Average values of parents
    return child

def mutate_hyperparameters(hyperparameters, hyperparameter_space, mutation_rate=0.1):
    """Mutate hyperparameters by adding random noise."""
    for key, (low, high) in hyperparameter_space.items():
        if random.random() < mutation_rate:
            hyperparameters[key] += random.uniform(-0.1, 0.1) * (high - low)
            hyperparameters[key] = max(low, min(high, hyperparameters[key]))  # Clamp to range
...
if __name__ == "__main__":
    ...
    # Optimize hyperparameters
    print("Optimizing hyperparameters by training a new model each time...")
    best_hyperparameters = optimize_hyperparameters(POPULATION_SIZE, GENERATIONS, TRAINING_MAZES)

    print("\nBest Hyperparameters Found:")
    print(best_hyperparameters)

    # Test the best hyperparameters
    print("\nTesting the best hyperparameters on unseen test maze...")
    best_agent = Agent(
        state_size=2,
        action_size=4,
    ...
    train_agents_on_maze([best_agent], TEST_MAZE, TRAINING_EPISODES)
